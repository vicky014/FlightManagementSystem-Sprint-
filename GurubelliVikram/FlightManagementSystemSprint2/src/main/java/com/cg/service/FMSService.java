...
import com.cg.exception.UserCreatedException;
...
public class FMSService {
	...
	//Flight
			...
			public Flight viewFlight(String flightNumber) throws FMSException {
				
				
				List<Flight> list=viewFlight();
				Optional <Flight> optional=list.stream().
						filter(f1->f1.getFlightNumber().equalsIgnoreCase(flightNumber)).findFirst();
...
//				
			...
			public String addFlight(Flight flight) throws FMSException {
				...
				List<Flight> list=viewFlight();
				Optional <Flight> optional=list.stream().
						filter(f1->f1.getFlightNumber().equalsIgnoreCase(flight.getFlightNumber()) ).findFirst();
...
//				return flightDao.addFlight(flight);
	...
	//ScheduledFlight
			...
			public ScheduledFlight viewScheduledFlight(String flightNumber) throws UserCreatedException {
				
				List<ScheduledFlight> list=viewScheduledFlights();
				Optional <ScheduledFlight> optional=list.stream().
						filter(a1->a1.getFlight().getFlightNumber().equalsIgnoreCase(flightNumber)).findFirst();
				...
				if(optional.isPresent())
				{
					...
					throw new UserCreatedException("Flightnumber does not exist");
			...
			public String scheduleFlight(ScheduledFlight scheduledFlight) throws UserCreatedException {
...
//					return  daoInt.scheduleFlight(scheduledFlight);
				try {
				daoInt.scheduleFlight(scheduledFlight);
				}
				catch(DataIntegrityViolationException e) {
					throw new UserCreatedException("Flight ID and Schedule ID and Scheduled Flight already taken try new one");
				}
			return "added";	
			//return daoInt.scheduleFlight(scheduledFlight);
			}
			
//			public String modifyScheduledFlight(Flight flight,Schedule schedule,String flightNumber) {
...
//			}
	...
	//Schedule
			...
			public Schedule viewSchedule(String scheduleId)
			{
				return daoInt.viewSchedule(scheduleId);
			}
//			
